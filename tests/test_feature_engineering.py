...
def add_day_length(dataframe: pd.DataFrame, longitude: float, latitude: float) -> pd.DataFrame:
    ...
    for date in period:
        ...
        daylengths.append(
            (times['sunset'] - times['sunrise']).total_seconds() / 3600) # type: ignore
...
def generate_cyclic_features(
        dataframe: pd.DataFrame, col_names: list[str], periods: list[int], start_nums: list[int]):
    ...
    for col_name, period, start_num in zip(col_names, periods, start_nums):
        ...
        transform = {
            f'sin_{col_name}' : lambda x: np.sin(2*np.pi*(dataframe[col_name]-start_num)/period),
            f'cos_{col_name}' : lambda x: np.cos(2*np.pi*(dataframe[col_name]-start_num)/period)
        }
...
def main() -> None:
    lon = 9.58
    lat = 55.73
    data = pd.read_csv('data/load_data.csv', parse_dates=True, index_col='datetime')
    dls = add_day_length(data, lon, lat)
    holidays_dk = holidays.DK()
    hlds = add_holiday(dls, holidays_dk)
    col_names = ['hour', 'month']
    temp = add_temporal(hlds, col_names)
    periods = [24, 12]
    start_nums = [0, 1]
    cyclic = generate_cyclic_features(temp, col_names, periods, start_nums)
    sun = add_sun_position(cyclic, lon, lat)
    weekend = add_weekends(sun)
    print(weekend.head(50))
